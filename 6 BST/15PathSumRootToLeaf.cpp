...
int heightTree1(BinaryTreeNode<int> *root)
{
    if(root == NULL)
    {
        return 0 ;
    ...
    return 1+max(heightTree1(root -> left) , heightTree1(root -> right)) ;
}
...
int heightOfLargestBST(BinaryTreeNode<int> *root)
{
    ...
    if(checkBST(root))
    {
        return heightTree1(root) ;
    }
    return max(heightOfLargestBST(root -> left) , heightOfLargestBST(root -> right)) ;
    // BinaryTreeNode<int> *temp = root ;
    // while(!checkBST(root))
    // {
    //     root = root -> left ;
    // }
    // int a = heightTree1(root) ;
    // while(!checkBST(temp))
    // {
    //     temp = temp -> right ;
    // }
    // int b = heightTree1(temp) ;
    // return max(a,b) ;
...
void pathSumRootToLeaf(BinaryTreeNode<int> *root , int k , string &s)
{
    if(root == NULL)
    {
        return ;
...
//8 9 10 3 7 -1 -1 -1 -1 6 11 5 12 -1 13 -1 -1 4 14 -1 -1 1  -1 -1 -1 -1 -1
