...
BinaryTreeNode<int> *Helper2(int *in , int *pos , int inS , int inE , int posS , int posE)
{
    // cout<<"Here" ;
    if(inS > inE)
    {
    ...
    BinaryTreeNode<int> *root = new BinaryTreeNode<int>(rootData) ;
    root -> left = Helper2(in , pos , lInS , lInE , lPosS , lPosE) ;
    root -> right = Helper2(in , pos , rInS , rInE , rPosS , rPosE) ;
...
BinaryTreeNode<int> *buildTreeFromPostorder_Inorder(int *in , int *pos , int size)
{
    return Helper2(in , pos , 0 , size-1 , 0 , size-1);
}
...
int main()
{
    ...
    int in[] =   {2  , 9 , 3  , 6 , 10 , 5} ;
    int pos[] = {2  , 6 , 3 , 9 , 5 , 10} ;
    int size = 6 ;
    BinaryTreeNode<int> *root1 = buildTreeFromPostorder_Inorder(in , pos , size) ;
