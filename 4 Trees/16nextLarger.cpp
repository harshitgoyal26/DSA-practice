...
void helperReplaceWithDepth(TreeNode<int> *root , int depth)
{
    if(root == NULL)
    {
        return;
    ...
    for(int i = 0 ; i  < root -> children.size() ; i++)
    {
        helperReplaceWithDepth(root -> children[i] , depth +1);
    }
}
...
int main()
{
    ...
    // cout<<areIdentical(root , root2)<<endl;

    cout<<nextLarger(root , 18) <<endl ;
